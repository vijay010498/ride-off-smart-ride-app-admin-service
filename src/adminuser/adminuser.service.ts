...
import {
  AdminUserTokenBlacklist,
  AdminUserTokenBlacklistDocument,
} from './adminuser-token-blacklist.schema';
...
export class AdminUserService {
  ...
  async createAdminUser(createAdminDto: CreateAdminUserDto) {
    ...
    if (!email || !firstName || !lastName || !userType) {
      throw new BadRequestException(
        'Missing required fields in CreateAdminUserDto',
      );
    }
    ...
    try {
      ...
      const notificationMessage =
        'Welcome' +
        firstName +
        ', The Password to your newly created account is : ' +
        plainPassword +
        '';
  ...
  async updatePassword(userId: string, updatePasswordDto: PasswordDto) {
    ...
    const newHashedPassword = bcrypt.hashSync(
      ...
      10,
    ); // Use a suitable salt rounds value
  ...
  async searchAdminUsers(
    ...
    const [users, totalUsers] = await Promise.all([
      this.adminModel
        .find(searchQuery, { hashedPassword: 0 })
        .skip(skip)
        .limit(limit)
        .sort({ _id: 'asc' }),
  ...
  async tokenInBlackList(accessToken: string) {
    const tokenInBlackList =
      await this.AdminUserTokenBlacklistCollection.findOne({
        token: accessToken,
      });
