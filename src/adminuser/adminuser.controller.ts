import { Body, Controller, Get, Param, Post, Patch, Query, UnauthorizedException, UseInterceptors, UseGuards } from '@nestjs/common';
...
export class AdminuserController {
    ...
    @Get()
    @UseGuards(AccessTokenGuard, IsBlockedGuard, TokenBlacklistGuard, AdminUserTypeGuard)
    async getAllAdminUsers(@Query('page') page: number, @Query('limit') limit: number) {
    ...
    @Patch(':id/password')
    @UseGuards(AccessTokenGuard, IsBlockedGuard, TokenBlacklistGuard, AdminUserTypeGuard)
    async updatePassword(@Param('id') userId: string, @Body() updatePasswordDto: PasswordDto) {
        return this.adminUserService.updatePassword(userId, updatePasswordDto);
    }

    @Get('search')
    @UseGuards(AccessTokenGuard, IsBlockedGuard, TokenBlacklistGuard, AdminUserTypeGuard)
    async searchAdminUsers(@Query() searchDto: SearchAdminUserDto, @Query('page') page: number, @Query('limit') limit: number) {
